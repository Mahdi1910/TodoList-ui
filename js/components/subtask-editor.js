window.SubtaskEditorComponent = {
  ...
  bindEvents() {
    ...
    this.modal?.addEventListener('keydown', event => {
      ...
      if (this.menuPriority?.classList.contains('open') || this.menuTags?.classList.contains('open')) this.closeMenus();
      else { event.preventDefault(); this.close(); }
    });
    this.btnDate?.addEventListener('click', () => this.openSchedule());
    this.btnPriority?.addEventListener('click', event => { event.stopPropagation(); this.toggleMenu(this.menuPriority); });
    this.btnTags?.addEventListener('click', event => { event.stopPropagation(); this.toggleMenu(this.menuTags); });
    this.menuPriority?.addEventListener('click', event => {
      const item = event.target.closest('[data-subtask-priority]');
      if (!item) return;
    ...
    this.menuTags?.addEventListener('click', event => {
      const item = event.target.closest('[data-subtask-tag]');
      if (!item) return;
      event.stopPropagation();
  ...
  close() {
    ...
    this.closeMenus();
    window.ModalFocusManager.close(this.modal, { fallbackFocus: '#btn-open-add-task' });
  ...
  syncPriorityUI() {
    this.menuPriority?.querySelectorAll('[data-subtask-priority]').forEach(item => item.classList.toggle('selected', item.dataset.subtaskPriority === this.selectedPriority));
    this.btnPriority?.classList.toggle('active', Boolean(this.selectedPriority));
  },

  syncTagUI() {
    this.menuTags?.querySelectorAll('[data-subtask-tag]').forEach(item => item.classList.toggle('selected', this.selectedTags.includes(item.dataset.subtaskTag)));
