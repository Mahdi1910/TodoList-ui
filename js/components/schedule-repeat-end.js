window.ScheduleRepeatEndMethods = {
  initRepeatEndUi() {
    ...
    modal.innerHTML = `
      <div class="modal-card repeat-end-card">
        <div class="modal-header"><h3 class="modal-title">Repeat Ends</h3><button type="button" class="modal-close-btn" data-repeat-end-close aria-label="Close repeat end dialog">&times;</button></div>
        <span class="repeat-end-label">End by</span>
        <div class="time-picker-container repeat-end-type-wheel" role="group" aria-label="Repeat ending mode"><div class="wheel-mask top" aria-hidden="true"></div><div class="wheel-mask bottom" aria-hidden="true"></div><div class="selection-highlight" aria-hidden="true"></div><div class="time-wheel" data-repeat-end-type-wheel role="listbox" tabindex="0" aria-label="Repeat ending mode"></div></div>
        <div class="repeat-end-conditional" data-repeat-end-date hidden><div class="calendar-header repeat-end-calendar-header"><button type="button" class="calendar-nav-btn" data-repeat-end-prev aria-label="Previous Month">‹</button><span class="calendar-title" data-repeat-end-month></span><button type="button" class="calendar-nav-btn" data-repeat-end-next aria-label="Next Month">›</button></div><div class="calendar-weekdays" aria-hidden="true"><span>Su</span><span>Mo</span><span>Tu</span><span>We</span><span>Th</span><span>Fr</span><span>Sa</span></div><div class="calendar-grid repeat-end-calendar-grid" data-repeat-end-grid role="grid" aria-label="Repeat end date"></div></div>
        <div class="repeat-end-conditional" data-repeat-end-count hidden><div class="time-picker-container repeat-end-count-wheel" role="group" aria-label="Repeat occurrence count"><div class="wheel-mask top" aria-hidden="true"></div><div class="wheel-mask bottom" aria-hidden="true"></div><div class="selection-highlight" aria-hidden="true"></div><div class="time-wheel" data-repeat-end-count-wheel role="listbox" tabindex="0" aria-label="Number of occurrences"></div></div><span class="repeat-end-count-label">times</span></div>
    ...
    window.ModalFocusManager.register(modal, row);
    ...
    this.populateWheel(this.repeatEndCountWheel, Array.from({ length: 200 }, (_, index) => String(index + 1)));
    ...
    this.bindRepeatEndWheel(this.repeatEndCountWheel, index => { if (this.repeatEndDraft) this.repeatEndDraft.count = index + 1; });
  ...
  bindRepeatEndWheel(wheel, onChange) {
    ...
    wheel.addEventListener('click', event => {
      ...
      if (!item) return;
      const index = [...wheel.querySelectorAll('.wheel-item')].indexOf(item);
  ...
  closeRepeatEndModal() {
    if (!this.repeatEndModal?.classList.contains('active')) return;
    window.ModalFocusManager.close(this.repeatEndModal, { fallbackFocus: this.repeatEndsRow });
  ...
  setRepeatEndType(type) {
    ...
    if (type === 'date') { this.repeatEndDraft.date ||= this.draftDate || window.RepeatEngine.today(); this.repeatEndDraft.count = null; }
  ...
  renderRepeatEndCalendar() {
    ...
    for (let index = first - 1; index >= 0; index--) this.repeatEndCalendarGrid.appendChild(this.createRepeatEndDay(year, month - 1, previous - index, true));
