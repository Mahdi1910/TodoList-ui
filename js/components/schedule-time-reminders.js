window.ScheduleTimeReminderMethods = {
  ...
  toggleReminderSelection(key) {
    if (key === 'none') this.draftReminders = ['none'];
      ...
      if (this.draftReminders.includes(key)) this.draftReminders = this.draftReminders.filter(value => value !== key);
      else this.draftReminders.push(key);
  ...
  renderReminderMenuContent() {
    ...
    this.menuReminder.querySelectorAll('.reminder-menu-item').forEach(item => {
      ...
      if (!key) return;
      const selected = this.draftReminders.includes(key);
    ...
    if (this.customRemindersContainer) {
      ...
      this.customReminders.forEach(custom => {
        ...
        div.className = `reminder-menu-item ${selected ? 'selected' : ''}`;
        div.dataset.reminder = custom.id;
        div.setAttribute('role', 'menuitemcheckbox');
        div.setAttribute('aria-checked', selected ? 'true' : 'false');
  ...
  updateReminderUI() {
    ...
    const labels = this.draftReminders.map(key => {
      ...
      return this.customReminders.find(custom => custom.id === key)?.label || key;
    });
  ...
  closeCustomReminderModal() {
    if (this.customReminderModal?.classList.contains('active')) {
      window.ModalFocusManager.close(this.customReminderModal, { fallbackFocus: this.btnOpenCustomReminder });
    }
  },
