(() => {
  ...
  async function persistTaskRows(tasks) {
    ...
    await window.TodoDb.withTransaction(S.TASKS, 'readwrite', async tx => {
      for (const task of tasks) {
        await window.TodoRepositories.put(tx, S.TASKS, window.TodoStorageMappers.taskToRow(task));
      }
    });
  }
  ...
  async function completeRepeatingSubtask(task) {
    ...
    if (!nextDate) {
      await persistAggregates([oldTask]);
      replaceMemory([oldTask]);
      return oldTask;
    }
    ...
    const nextTask = copyTask(task, {
      ...
      repeatState: nextState(task),
      createdAt: now,
      updatedAt: now
    });
    await persistAggregates([oldTask, nextTask]);
    replaceMemory([oldTask], [nextTask]);
    return nextTask;
  }

  async function completeNonRepeatingRoot(root) {
    const now = window.TodoStorageMappers.nowIso();
    const family = [root, ...window.AppState.getSubtasks(root.id)].map(task =>
      copyTask(task, { completed: true, updatedAt: now }));
    await persistTaskRows(family);
    replaceMemory(family);
    return family[0];
  }
  ...
  async function finishRepeatingRootWithoutNext(root, children) {
    ...
    await persistAggregates([oldRoot, ...oldChildren]);
    replaceMemory([oldRoot, ...oldChildren]);
    return oldRoot;
  }
  ...
  async function completeRepeatingRoot(root) {
    ...
    const nextRoot = copyTask(root, {
      ...
      repeatState: nextState(root),
      createdAt: now,
      updatedAt: now
    });

    const templates = chooseSlotTemplates(children);
    ...
    await persistAggregates([oldRoot, ...oldChildren, nextRoot, ...nextChildren]);
    replaceMemory([oldRoot, ...oldChildren], [nextRoot, ...nextChildren]);
    return nextRoot;
  }

  service.toggleTaskStatus = function repeatAwareToggleTaskStatus(taskId) {
    return this.enqueue(async () => {
      const task = window.AppState.getTask(taskId);
      if (!task) throw new Error('Task not found.');
      if (task.completed) return uncompleteTask(task);
      if (task.parentTaskId) {
        return activeRepeat(task) ? completeRepeatingSubtask(task) : completePlainSubtask(task);
      }
