window.TodoStorageMappers = (() => {
  ...
  function taskFromRow(row, tags = [], reminders = [], repeatData = null) {
    const repeat = repeatData?.repeat || null;
    const repeatState = repeatData?.repeatState ? { ...repeatData.repeatState } : null;
    let dueDate = row.dueDate || null;
    let normalizedRepeat = null;
    let normalizedState = null;
    if (repeat && repeat.mode !== 'none') {
      normalizedRepeat = window.RepeatEngine.normalizeRepeatRule(repeat);
      if (!dueDate) dueDate = window.RepeatEngine.today();
      normalizedState = window.RepeatEngine.createInitialRepeatState(normalizedRepeat, dueDate, repeatState || {});
      normalizedState.seriesId = repeatState?.seriesId || createId('series');
      if (repeatState?._needsRepair || !row.dueDate) normalizedState._needsRepair = true;
    }
    return {
      id: row.id,
      title: row.title,
  ...
  function repeatToRow(taskId, repeat, repeatState = null) {
    ...
    return {
      ...
      yearDates: custom.yearDates && typeof custom.yearDates === 'object' ? JSON.parse(JSON.stringify(custom.yearDates)) : {},
  ...
  function repeatFromRow(row) {
    if (!row) return { repeat: null, repeatState: null };
    const repeat = window.RepeatEngine.normalizeRepeatRule({
      ...
      custom: {
        ...
        yearDates: row.yearDates && typeof row.yearDates === 'object' ? JSON.parse(JSON.stringify(row.yearDates)) : {}
      },
      end: { type: row.endType || 'never', date: row.endDate || null, count: row.endCount ?? null }
    });
    const repeatState = {
    ...
    if (!row.seriesId || !row.anchorDate || !Number.isInteger(row.anchorDay) || !Number.isInteger(row.anchorMonth)) repeatState._needsRepair = true;
    return { repeat, repeatState };
  }

  function builtinDefinitions() {
    const createdAt = nowIso();
    return BUILTIN_REMINDERS.map(item => ({ ...item, type: 'builtin', isBuiltin: 1, createdAt }));
  }
  function customReminderToDefinition(custom) {
    ...
    return { id: custom.id, label: custom.label || custom.id, type: 'custom', minutesBefore, isBuiltin: 0, createdAt: custom.createdAt || nowIso() };
  }
  function definitionToCustomReminder(definition) {
    ...
    const day = Math.floor(total / 1440); total -= day * 1440;
    const hr = Math.floor(total / 60); const min = total - hr * 60;
    return { id: definition.id, label: definition.label, day, hr, min };
  }

  return { BUILTIN_REMINDERS, nowIso, taskToRow, taskFromRow, repeatToRow, repeatFromRow, builtinDefinitions, customReminderToDefinition, definitionToCustomReminder };
})();
