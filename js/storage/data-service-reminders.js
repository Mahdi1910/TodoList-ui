Object.assign(window.AppDataService, {
  saveReminderDefinition(custom) {
    return this.enqueue(async () => {
      ...
      await window.TodoDb.withTransaction(S.REMINDER_DEFINITIONS, 'readwrite', tx => window.TodoRepositories.put(tx, S.REMINDER_DEFINITIONS, definition));
      const index = window.AppState.reminderDefinitions.findIndex(item => item.id === definition.id);
      if (index >= 0) window.AppState.reminderDefinitions[index] = { ...definition };
      else window.AppState.reminderDefinitions.push({ ...definition });
      return definition;
    });
  },
  ...
  deleteReminderDefinition(reminderId) {
    return this.enqueue(async () => {
      ...
      window.AppState.reminderDefinitions = window.AppState.reminderDefinitions.filter(item => item.id !== reminderId);
