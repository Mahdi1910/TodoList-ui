window.AppDataService = {
  ...
  resolveReminders(reminders = []) {
    ...
    for (const id of ids) {
      ...
      const stored = window.AppState.reminderDefinitions.find(item => item.id === id && !item.isBuiltin);
      if (stored) { definitions.push({ ...stored }); continue; }
  ...
  buildTask(taskData = {}, existing = null) {
    ...
    const project = parent ? (parent.project || '') : this.validateProjectId(taskData.project ?? existing?.project ?? '');
